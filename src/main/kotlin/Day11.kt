...
var vertices = mutableMapOf<String, Verticle>()
...
fun main() {
    ...
    input.lines().forEach { line ->
        var v = Verticle(line.split(':').first())
        vertices[v.name] = v
    }
    vertices["out"] = Verticle("out")
    input.lines().forEach { line ->
        line.split(':').zipWithNext().forEach { (verticle, neighbors) ->
            neighbors.trim().split(" ").forEach {
                vertices[verticle]!!.neighbors += vertices[it]!!
            }
        }
    }

    countPaths[vertices["out"]!!] = PathValues(0, 0, 0, 1)
    var result = rec(vertices["svr"]!!, vertices["fft"]!!, vertices["dac"]!!)
    println(result)

}

var countPaths = mutableMapOf<Verticle, PathValues>()

fun rec(start: Verticle, fft: Verticle, dac: Verticle): PathValues {
    if (countPaths[start] != null) return countPaths[start]!!

    var result = PathValues(0, 0, 0, 0);
    for (neigher in start.neighbors) {
        result += rec(neigher, fft, dac);
    }
    if (start == dac) {
        result.withoutAny += result.withDAC
        result.withFFT += result.withDACAndFFT

    } else if (start == fft) {
        result.withoutAny += result.withFFT
        result.withDAC += result.withDACAndFFT
    }
    countPaths[start] = result
    return result
}


class Verticle(var name: String, var neighbors: Set<Verticle>) {
    constructor(name: String) : this(name, mutableSetOf())
}

class PathValues(var withoutAny: Long, var withFFT: Long, var withDAC: Long, var withDACAndFFT: Long) {
    operator fun plusAssign(other: PathValues) {
        this.withoutAny += other.withoutAny
        this.withDAC += other.withDAC
        this.withFFT += other.withFFT
        this.withDACAndFFT += other.withDACAndFFT

    }

    override fun toString(): String {
        return "$withoutAny/$withFFT/$withDAC/$withDACAndFFT"
    }
}
